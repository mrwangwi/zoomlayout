...
constructor(context: Context) : ZoomApi {
    ...
    override fun realZoomTo(@RealZoom realZoom: Float, animate: Boolean) {
        val update = MatrixUpdate.obtain { zoomTo(realZoom, true) }
        if (animate) {
            matrixController.animateUpdate(update)
        } else {
            cancelAnimations()
    ...
    fun setEndListener(endListener: ZoomLayout.EndListener): Boolean {
        return matrixController.setEndListener(endListener)
    }

    fun removeEndListener() {
        matrixController.removeEndListener()
    }
